...
values() 方法返回数组的值迭代器。可以在 for of 迭代结构中使用。
...
for (const key of persons.values()) {
  console.log(key);
}
// -> { name: 'alice', age: 18 }
// -> { name: 'bob', age: 20 }
...
reduce() 方法用于归并操作。传入的第 1 个参数为函数，具有参数 (preEle, curElem)，第 2 个参数称为归并初始值。reduce() 使用传入的函数从第1个元素（提供初始值，则从初始值开始）开始归并，每次归并的结果就是传入函数的返回值，而每次归并的结果都会成为下一次归并函数的 preEle 参数。
